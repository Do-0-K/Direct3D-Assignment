...
public:
	...
	void CreateBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pData, UINT nElements, UINT nStride, DXGI_FORMAT dxgiFormat, D3D12_HEAP_TYPE d3dHeapType, D3D12_RESOURCE_STATES d3dResourceStates, UINT nIndex , int);
	...
	int LoadTextureFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, CGameObject* pParent, FILE* pInFile, CShader* pShader, UINT nIndex, int change);
...
public:
	...
	BoundingOrientedBox skymap_box = BoundingOrientedBox();
...
public:
	...
	void SetMaterial(CMaterial* pMeterial);
	...
	void MovingDirection(XMFLOAT3& xmf3MovingDirection) { m_xmf3MovingDirection = Vector3::Normalize(xmf3MovingDirection); }
	void setMovingSpeed(float xx) { m_mspeed = xx; }
...
public:
	...
	static CGameObject* LoadFrameHierarchyFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, CGameObject* pParent, FILE* pInFile, CShader* pShader, int change);
	static CGameObject* LoadGeometryFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, char* pstrFileName, CShader* pShader, int change);
...
{
public:
	CBulletObject(CubeMesh* sharedCubeMesh, CMaterial* sharedCubeMaterial);
...
private:
	...
	int shoot_time = 0;
	float						m_fBulletEffectiveRange = 75.0f;
	CGameObject* m_pLockedObject = NULL;
public:
	virtual void PrepareAnimate();
	virtual void Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent = NULL);
	virtual void FireBullet(CGameObject* m_pLockedObject);
	virtual void Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera = NULL);
};
...
class TwoDText : public CGameObject
{
public:
	TwoDText(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature);
	virtual ~TwoDText();
...
class MenuText : public CGameObject
{
public:
	MenuText(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature);
	virtual ~MenuText();
};

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

class CheckText : public CGameObject
{
public:
	CheckText(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, ID3D12RootSignature * pd3dGraphicsRootSignature);
	virtual ~CheckText();
};
...
class Skymap : public CGameObject
{
public:
	Skymap(CubeMesh* sharedCubeMesh, CMaterial* sharedCubeMaterial);
	virtual ~Skymap();
};
