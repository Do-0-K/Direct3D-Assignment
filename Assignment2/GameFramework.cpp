...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	RECT Start= { 330, 340, 680, 420 }; //시작화면 시작 버튼
	RECT GoMenu = { 10, 340,310,420 }; //시작화면 조작법 및 설정 버튼
	RECT Back = { 440, 320,680,460 }; //메뉴화면 뒤로가기 버튼
	...
	if (change == 0) {
		...
		case WM_LBUTTONUP:
			...
			if (mouseX >= Start.left && mouseX <= Start.right &&
				mouseY >= Start.top && mouseY <= Start.bottom)
			{
				change = 1;
				ReleaseObjects();
				BuildObjects();
				m_pCamera = m_pPlayer->ChangeCamera((DWORD)(VK_F3 - VK_F1 + 1), m_GameTimer.GetTimeElapsed());
			}
			if (mouseX >= GoMenu.left && mouseX <= GoMenu.right &&
				mouseY >= GoMenu.top && mouseY <= GoMenu.bottom)
			{
				change = 2;
				ReleaseObjects();
				BuildObjects();
		...
		case WM_LBUTTONUP:
			...
			if (mouseX >= Back.left && mouseX <= Back.right &&
				mouseY >= Back.top && mouseY <= Back.bottom)
			{
				change = 0;
				ReleaseObjects();
				BuildObjects();
...
void CGameFramework::BuildObjects()
{
	...
	if (change == 0) { //시작화면
		...
		CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature(), change);
		pAirplanePlayer->SetPosition(XMFLOAT3(0.0f, 0.0f, -20.0f));
		m_pScene->m_pPlayer = m_pPlayer = pAirplanePlayer;
		m_pPlayer->SetPlayerUpdatedContext(NULL);
		m_pPlayer->SetCameraUpdatedContext(NULL);
		m_pCamera = m_pPlayer->GetCamera();
	}
	else if (change == 2) { //설정 및 조작법 화면
		m_pScene = new Menu();
		if (m_pScene) m_pScene->BuildObjects(m_pd3dDevice, m_pd3dCommandList, 0);
		...
		m_pScene = new CScene(); //인게임 화면
		if (m_pScene) m_pScene->BuildObjects(m_pd3dDevice, m_pd3dCommandList, Objectnum);

		CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature(), change);
		pAirplanePlayer->SetPosition(XMFLOAT3(1400.0f, 400.0f, 1500.0f));
...
void CGameFramework::CreateShaderVariables()
{
	...
	m_pd3dcbFrameworkInfo = ::CreateBufferResource(m_pd3dDevice, m_pd3dCommandList, NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER | D3D12_RESOURCE_STATE_GENERIC_READ, NULL,1);
